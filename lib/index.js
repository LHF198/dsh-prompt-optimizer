...
import { TimedAbortController } from './abort.js'
...
const OPTIMIZER_SYSTEM_BASE =
  '你是一个提示词优化器。请将用户提供的原始输入改写成清晰、具体、可执行的提示词。' +
  '补充目标、必要的上下文、约束条件和期望输出格式，但不得虚构事实、背景、数据或用户未表达的要求。' +
  '只输出优化后的提示词，不要解释修改过程，不要加引号。'

const STYLE_HINTS = {
  general: '',
  professional: '采用更专业、正式的措辞，突出严谨与可操作性。',
  concise: '尽量精简，去除冗余修饰，保留关键信息与结构。',
  coding: '面向编程任务：明确输入/输出、约束、边界条件与验收标准。',
}

const LANG_HINTS = {
  auto: '输出语言与原始输入的主要语言保持一致。',
  zh: '一律输出中文。',
  en: '一律输出英文。',
}

/** Compose the optimizer system prompt from a style and language selection. */
function buildSystem(style, language) {
  const s = STYLE_HINTS[style] || STYLE_HINTS.general
  const l = LANG_HINTS[language] || LANG_HINTS.auto
  return OPTIMIZER_SYSTEM_BASE + s + l
}
...
function apply(ctx) {
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      const style = typeof body.style === 'string' ? body.style : 'general'
      ...
      const opts = {
        ...
        system: buildSystem(style, language),
        maxTokens: 4096,
      }
      if (pick.reasoningEffort !== undefined) opts.reasoningEffort = pick.reasoningEffort

      const ctrl = typeof AbortController !== 'undefined' ? new TimedAbortController(MODEL_TIMEOUT_MS) : null
      if (ctrl !== null) opts.signal = ctrl.signal
