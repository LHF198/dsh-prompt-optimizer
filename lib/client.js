  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ctx.effect(function* () {
        ...
        var state = { sessionId: undefined, open: false, busy: false, phase: "idle", original: "", result: "", error: "", request: 0, lastDraft: "", inputActions: null, timerDisposer: null, listeners: new Set() };
        ...
        function syncFrom(p) {
          ...
          if (sid && sid !== state.sessionId) {
            ...
            state.original = ""; state.result = ""; state.error = "";
          }
          var d = draftOf(p);
        ...
        function start() {
          ...
          state.timerDisposer = ctx.timer.timeout(function () {
            if (n === state.request && state.busy && state.phase === "running") {
              ++state.request; clearTimer(); state.busy = false; state.open = false; state.phase = "error"; state.error = t("error.timeout"); state.emit();
            }
          }, TIMEOUT_MS);
          fetch(API, {
            method: "POST",
            ...
            .catch(function () {
              if (n !== state.request) return;
              clearTimer(); state.busy = false; state.phase = "error"; state.error = t("error.generic"); state.emit();
            });
        }
